...
## Региональные пробники (MVP)

Облачный xray-checker сидит вне зоны блокировок и не видит ban'ы по фингерпринту/SNI/IP,
которые активны только в РФ. Получается: сервер пишет «всё ОК», но реальные пользователи
из РФ подключиться не могут. Чтобы такие случаи видеть, можно поднять **probe-агентов**
на устройствах в зоне блокировки — они локально делают TLS handshake к каждому
прокси-конфигу и репортят результат на статус-страницу.

На странице у каждого сервера появится строка с маркерами: «🟢 mac-home 47 ms» (пробник
видит сервер), «🔴 mac-home» (не видит — наведи мышью, в тултипе будет ошибка).
...
# спросит: URL статус-страницы, URL подписки, ADMIN_TOKEN, имя пробника
```
...
**Снести:** `bash install-macos.sh --uninstall`.
